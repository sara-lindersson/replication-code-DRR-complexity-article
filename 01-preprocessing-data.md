...
This script performs the pre-processing of data for Lindersson et al
2025. The script pre-processes the collection of case studies by tidying
the dataframe and identifying the coordinates of each case study using
administrative unit IDs (`gid`) from [GADM
v3.6](https://gadm.org/download_world36.html). The script also performs
outlier analysis. The output from this script is the `source_data`: one
csv-file and one R dataframe.
...
### Pre-proecessing parameters

``` r
table_file <- here("raw-data", "table-20250307.csv")
...
```
    ...
    ## [1] "The identified outlier is case: 2017 bushfires in NSW, Australia"
