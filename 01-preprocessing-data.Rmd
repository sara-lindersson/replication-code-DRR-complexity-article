...
This script performs the pre-processing of data for Lindersson et al 2025. 
The script pre-processes the collection of case studies by tidying the dataframe and identifying the coordinates of each case study using administrative unit IDs (`gid`) from [GADM v3.6](https://gadm.org/download_world36.html). The script also performs outlier analysis. The output from this script is the `source_data`: one csv-file and one R dataframe.
...
```{r load-tidy-data, eval=T, warning=FALSE, include=T}
df <- read.csv(
  table_file,
  header = TRUE,
  na.strings = c("NA", "")
...
### Identify centroids
```{r identify-centroids, eval=T, include=T, message=F, results=F}
